...
public class Lobster_traptile extends TileEntity implements IInventory {
	...
	private String name="Lobster trap";
...
ItemStack stack = getStackInSlot(i);
	...
	public void addFish(int fishamout, int neededwater, int range, int waterforextrafish) {
	...
	protected class Coordinateset implements Set<Cordinate>{
		private ArrayList<Cordinate> list= new ArrayList<Lobster_traptile.Cordinate>();
		...
		private boolean isAlredayInList(Cordinate e) {
			for (Iterator<Cordinate> iterator = list.iterator(); iterator.hasNext();) {
				Cordinate cordinate = (Cordinate) iterator.next();
		...
		for (int i = 1; i < range+1; i++) {
			if (worldObj.getBlockId(xCoord+i, yCoord, zCoord)==9) {
				...
				if(worldObj.getBlockId(xCoord+i, yCoord+1, zCoord)==9){
					waterblocks.add(new Cordinate(xCoord+i, yCoord+1, zCoord));
				}
				if(worldObj.getBlockId(xCoord+i, yCoord-1, zCoord)==9){
					waterblocks.add(new Cordinate(xCoord+i, yCoord-1, zCoord));
				}
				if(worldObj.getBlockId(xCoord+i, yCoord, zCoord-1)==9){
					waterblocks.add(new Cordinate(xCoord+i, yCoord, zCoord-1));
				}
				if(worldObj.getBlockId(xCoord+i, yCoord, zCoord+1)==9){
					waterblocks.add(new Cordinate(xCoord+i, yCoord, zCoord+1));
				}
		...
		for (int i = 1; i < range+1; i++) {
			if (worldObj.getBlockId(xCoord, yCoord+i, zCoord)==9) {
				...
				if(worldObj.getBlockId(xCoord+1, yCoord+i, zCoord)==9){
					waterblocks.add(new Cordinate(xCoord+1, yCoord+i, zCoord));
				}
				if(worldObj.getBlockId(xCoord-1, yCoord+i, zCoord)==9){
					waterblocks.add(new Cordinate(xCoord-1, yCoord+i, zCoord));
				}
				if(worldObj.getBlockId(xCoord, yCoord+i, zCoord-1)==9){
					waterblocks.add(new Cordinate(xCoord, yCoord+i, zCoord-1));
				}
				if(worldObj.getBlockId(xCoord, yCoord+i, zCoord+1)==9){
					waterblocks.add(new Cordinate(xCoord, yCoord+i, zCoord+1));
				}
		...
		for (int i = 1; i < range+1; i++) {
			if (worldObj.getBlockId(xCoord, yCoord, zCoord-i)==9) {
				...
				if(worldObj.getBlockId(xCoord+1, yCoord, zCoord-i)==9){
					waterblocks.add(new Cordinate(xCoord+1, yCoord, zCoord-i));
				}
				if(worldObj.getBlockId(xCoord-1, yCoord, zCoord-i)==9){
					waterblocks.add(new Cordinate(xCoord-1, yCoord, zCoord-i));
				}
				if(worldObj.getBlockId(xCoord, yCoord+1, zCoord-i)==9){
					waterblocks.add(new Cordinate(xCoord, yCoord+1, zCoord-i));
				}
				if(worldObj.getBlockId(xCoord, yCoord-1, zCoord-i)==9){
					waterblocks.add(new Cordinate(xCoord, yCoord-1, zCoord-i));
				}
