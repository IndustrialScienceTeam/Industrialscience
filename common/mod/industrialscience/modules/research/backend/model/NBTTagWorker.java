...
public static void writeResearchesToNBT(NBTTagCompound compund, Research[] researches){
...
public static NBTTagList getStepsAsNBTTag(Hashtable<Integer, Boolean> hashtable) {
	NBTTagList stepsNBT= new NBTTagList();
	for (Hashtable<Integer, Boolean> elements =hashtable;elements.keys().hasMoreElements();) {
...
public static Research readResearchfromNBTTag(NBTBase nbt) {
	...
	Research research=rf.getResearch(compound.getName(),readStepsfromNBTTag(compound.getTagList("Steps")));
	return research;
}
private static Hashtable<Integer, Boolean> readStepsfromNBTTag(
	...
	for (int i = 0; i < tagList.tagCount(); i++) {
		...
		steps.put(i,((NBTTagCompound)tagList.tagAt(i)).getBoolean("Enabled"));
	}
	return steps;
}
