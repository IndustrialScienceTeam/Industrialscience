...
public class BlockIronFurnaceController extends IBlockMultiBlockController {
    public BlockIronFurnaceController() {
        super(Material.rock, MultiBlockStructure.FURNACE_TIER1,IndustrialScience.MODID + ":" + IndustrialScience.getInstance().getFactoryModule().getBlockIronhull().getUnlocalizedName().substring(5));
        ValidBlocks=new Block[]{this,IndustrialScience.getInstance().getFactoryModule().getBlockIronhull()};
        this.setCreativeTab(IndustrialScience.getInstance().getCreativetab());
        setBlockName("tier1ironfurnace");
        setHardness(3.0F);
    }
